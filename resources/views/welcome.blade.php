...
<body class="bg-gray-100">
    <div class="flex items-center justify-center h-screen">
        <div class="bg-white p-6 rounded-lg shadow-md">
            ...
            <form action="{{ route('products.create') }}" method="GET" class="mt-2">
                <button type="submit" class="w-full px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-500 transition duration-200">
                    Ajouter un Produit
                </button>
            </form>

            <form action="{{ route('products.edit', ['product' => 1]) }}" method="GET" class="mt-2"> <!-- Remplacez 1 par un ID valide ou gérez-le dynamiquement -->
            ...
            <form action="{{ route('products.destroy', ['product' => 1]) }}" method="POST" class="mt-2"> <!-- Remplacez 1 par un ID valide -->
                @csrf
                @method('DELETE')
